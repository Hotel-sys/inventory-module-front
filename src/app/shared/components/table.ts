...
export class UbTableDirective {
  ...
  protected computedClass = computed(() => cn('w-full caption-bottom text-sm', this.class()));
}
...
export class UbTableBodyDirective {
  readonly class = input<string>('');
  protected computedClass = computed(() => cn('[&_tr:last-child]:border-0', this.class()));
}
...
export class UbTableFooterDirective {
  readonly class = input<string>('');
  protected computedClass = computed(() => cn('bg-muted/50 border-t font-medium [&>tr]:last:border-b-0', this.class()));
}
...
export class UbTableCellDirective {
  readonly class = input<string>('');
  protected computedClass = computed(() =>
    cn('p-2 align-middle [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]', this.class()),
  );
}
...
export class UbTableCaptionDirective {
  readonly class = input<string>('');
  protected computedClass = computed(() => cn('text-muted-foreground mt-4 text-sm', this.class()));
}
